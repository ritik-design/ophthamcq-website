...
primary_keyword: "how to make ophthalmology notes"
target_page: "/notes"
...
1. **The trigger:** the exact question, confusion or recall failure.
2. **The compressed repair:** a source-checked distinction, diagram, table cell or calculation.
3. **The prompt:** a way to hide the answer and retrieve it later.
...
1. **Review the new captures.** Delete anything that is merely copied background.
2. **Test before editing.** Cover every answer first; do not improve formatting before you know whether it fails.
3. **Promote recurrent errors.** A fact missed twice gets a clearer prompt, a visual cue or a place in the next mixed set.
4. **Archive mastered items.** Keep a source trail, but move repeatedly correct cards out of the immediate stack.
5. **Check unstable facts.** Flag items tied to a new guideline, drug, device, trial, fee or exam rule for a final source check.
6. **Choose the next week’s error targets.** Pick a small, named set rather than reopening every subject.
...
1. Can I identify the source and tell whether the fact needs a current re-check?
2. Does this page repair a real error or an exam-relevant decision?
3. Can I hide the answer and retrieve it in under two minutes?
4. Is there a date or event that will make me test it again?
...
- [Dunlosky et al., “Improving Students’ Learning With Effective Learning Techniques” (PMC)](https://pmc.ncbi.nlm.nih.gov/articles/PMC3741794/) — evidence review used for the retrieval-practice and distributed-practice context; accessed 18 August 2026.
- [Karpicke & Blunt, “Retrieval Practice Produces More Learning than Elaborative Studying with Concept Mapping” (PubMed)](https://pubmed.ncbi.nlm.nih.gov/20847239/) — original experimental record used with its study-condition limits; accessed 18 August 2026.
- [Wammes, Meade & Fernandes, “The Drawing Effect” (PubMed)](https://pubmed.ncbi.nlm.nih.gov/26017356/) — experimental memory study used only for the conditional drawing discussion; accessed 18 August 2026.
- [Mueller & Oppenheimer, “The Pen Is Mightier Than the Keyboard” (PubMed)](https://pubmed.ncbi.nlm.nih.gov/24760141/) — boundary source for the conditional warning about verbatim laptop notes; accessed 18 August 2026.

## Editorial QA and link manifest

- **Cannibalisation:** KEEP SEPARATE from `/notes`; this is an error-triggered personal note-building workflow, while the category is a resource destination.
- **Differentiation:** five note shapes matched to concrete ophthalmology tasks; source → compression → recall → retest loop; weekly deletion/triage rule.
- **Claim controls:** no pass, score, universal-medium, product-learning, update, price, app, user-count or affiliation claim. Learning evidence is explicitly conditional and not ophthalmology-exam outcome evidence.
- **Clinical boundary:** postgraduate examination education only; source checks are recommended, not patient-management instructions.
- **Outbound internal links:** `/free-mcqs`, `/free-mcqs/high-yield`, `/glossary`, `/products/optics-notes`, `/notes`, `/products/anatomy-notes`, `/resources` — verified against inventory and routes on 18 August 2026.
- **Inbound opportunities after publication and sitemap verification:** `/notes`, `/resources`, `/free-mcqs`, relevant topic-product pages.
