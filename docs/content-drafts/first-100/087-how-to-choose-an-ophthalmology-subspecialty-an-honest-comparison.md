...
primary_keyword: "ophthalmology subspecialty choice"
secondary_keywords: ["which ophthalmology fellowship to choose", "ophthalmology subspecialties comparison", "how to choose an ophthalmology fellowship"]
...
1. What proportion of time is clinic, diagnostics, procedures, theatre, ward work and calls?
2. Which patients return repeatedly, and what must the clinician notice at those return visits?
3. What part of the work does the fellow actually perform under supervision, rather than only observe?
4. Which other services, imaging teams, anaesthetists or rehabilitation professionals does the work depend on?
5. What work felt absorbing after a long day, and what work did you avoid reading about?
6. Could you name two supervisors and a service where you could learn the work safely?

The final question is deliberately practical. Fellowship structures are institution-specific. Some systems use formal match processes while others use direct institutional applications; neither a match directory nor a famous unit guarantees that a given post has the same exposure, supervision or eligibility in the next cycle.[^match]
...
Career choice literature in medicine commonly identifies personal interest, mentoring, perceived work characteristics and exposure as influences; it cannot calculate the correct specialty for one person.[^career] Turn those broad factors into evidence from your own rotations.
...
- **Cannibalisation:** KEEP SEPARATE from `/exams/fellowship`. That page is fellowship-exam study support; this is decision support before choosing a track or programme.
- **Differentiation:** an ordinary-week field log plus an evidence-not-assertion scorecard, rather than a prestige, salary or lifestyle ranking.
- **Outbound internal links:** `/exams/fellowship`, `/osce` — verified repository routes.
- **Inbound opportunities after publication:** `/exams/fellowship`, `/resources`, `/topics`, `/osce`.
- **Claim check:** no salary, job-market, lifestyle, fellowship-placement, visa or career-outcome guarantee; all programme-specific facts require final live verification.
...
[^match]: SF Match. [Ophthalmology Fellowship Match](https://www.sfmatch.org/specialty/ophthalmology-fellowship-match/). Used only as a live example that application systems can be process-specific; eligibility and programme details must be checked directly.
[^career]: Querido SJ, et al. [Factors influencing career choice in medicine](https://pubmed.ncbi.nlm.nih.gov/21169754/). Used as a general career-decision evidence boundary, not as an ophthalmology-specific prediction model.
