...
primary_keyword: "ophthalmology instruments viva"
target_page: "/products/instruments-in-ophthalmology-notes"
...
- [Ophthalmic instrumentation — PubMed-indexed review](https://pubmed.ncbi.nlm.nih.gov/9326965/) (accessed 18 August 2026).
- [Toxic anterior segment syndrome: role of enzymatic detergents used in cleaning intraocular surgical instruments — peer-reviewed article](https://pubmed.ncbi.nlm.nih.gov/27697240/) (accessed 18 August 2026).
- [American Academy of Ophthalmology education catalogue](https://www.aao.org/education/education-browse) (accessed 18 August 2026).

## Editorial QA and link manifest

- **Intent and scope:** practical-viva recognition for PG doctors. “Complete” is defined as a core exam tray, not every instrument in every subspecialty service.
- **Information gain:** seven-question visual sort plus name–design clue–role–confusion pair answer framework; categories permit an uncertainty-safe answer when the exact eponym is not known.
- **Clinical boundary:** no patient advice, surgical sequence, patient-specific indication, equipment assembly or reprocessing instruction. Local policy, manufacturer instructions and supervision govern real use.
- **Claim check:** instrument functions are described at broad role level; no pass guarantee, product efficacy claim, affiliation claim, user statistic or unverified company fact.
- **Source check:** all three external URLs were live-checked on 18 August 2026 and are cited for claims used in the prose. The AAO link is a learning gateway, not a clinical guideline.
- **Outbound internal links:** `/products/instruments-in-ophthalmology-notes` (exact-topic route, P0); `/products/instruments-drugs-practical-viva` (practical companion, P1); `/osce` (practical-viva hub, P0); `/glossary` (terminology, P3). All are verified `.org` routes.
- **Inbound opportunities:** `/osce` in an instrument-station section; `/products/instruments-in-ophthalmology-notes` and `/products/instruments-drugs-practical-viva` around sample/revision context; `/resources` in a practical-revision route. Recommendations only; no unrelated page edits made.
