...
primary_keyword: "retinal detachment case presentation"
secondary_keywords: ["retinal detachment long case", "retinal detachment viva", "retinal detachment examination"]
target_page: "/osce"
...
Retinal detachment describes separation of neurosensory retina from the retinal pigment epithelium. The three broad mechanisms are rhegmatogenous, tractional and exudative; mixed mechanisms are possible.[^statpearls] In a long case, identifying the mechanism is useful only if you then point to the supporting features.
...
1. **Confirm the detached retina and its boundaries.** State where attached retina becomes detached and whether the configuration is shallow, bullous, concave or fixed. Do not confuse a descriptive appearance with a final mechanism.
2. **Locate the macula.** If it is visible, say whether it is involved. If not, state the limitation rather than guessing. OCT, when available in the case, may clarify a macula whose status is difficult to assess clinically; say it is evidence you would use, not a routine answer to every station.[^statpearls]
3. **Search systematically for breaks.** Take the examiner through the periphery in a reproducible order. Record number, clock hour, type and any relation to lattice, atrophy, previous laser or detached retina. A break can be small; the exam reward is the disciplined search, not false certainty.
4. **Look for PVR and traction.** PVR is clinically important because membrane formation and retinal contraction can complicate repair. Describe folds, membranes, stiffness, shortening or a fixed configuration before using a classification label.[^pvr]
5. **Examine the fellow eye.** The correct detail depends on the case. A normal fellow eye can be stated briefly; relevant lattice, tears, prior retinopexy or lens status should be included explicitly.
...
The phrase “subject to complete examination” is not evasive. It shows that you understand diagnostic confidence has a limit. The NCBI review also notes that combinations can occur, so a tidy three-box classification should not make you ignore contradictory signs.[^statpearls]

## How to discuss management without overreaching

For an exam case, management discussion should start with priorities and determinants, not an operation name. State that a suspected retinal detachment needs prompt specialist assessment. Then identify the features that a vitreoretinal team uses to individualise the approach: mechanism, break number and position, extent, macular status, lens status, PVR, media clarity, fellow-eye findings, previous surgery and the patient’s broader context. Surgical approaches for rhegmatogenous RD may include pneumatic retinopexy, scleral buckling, pars plana vitrectomy or a combination; their selection is case-specific.[^statpearls]
...
| Source section | Destination | Destination title | Anchor | Type | Priority | Why it helps |
|---|---|---|---|---|---|---|
| Practical long-case checklist | `/topics/retina` | Retina MCQs & Study Guide | Retina study guide | BODY_CONTEXTUAL | P1 | supports terminology and pattern revision |
| Practical long-case checklist | `/products/retina-deciphered` | Retina Deciphered | Retina Deciphered | BODY_CONTEXTUAL | P0 | optional retina-specific revision resource |
| Practical long-case checklist | `/osce` | OSCE, Practical & Viva Voce Ready Bundle | OSCE, Practical & Viva Voce Ready bundle | CTA | P0 | matching practical/viva route |
| Practical long-case checklist | `/exams/dnb` | DNB Ophthalmology Preparation | DNB Ophthalmology preparation guide | BODY_CONTEXTUAL | P0 | relevant practical-preparation hub |

**Inbound opportunities:** `/topics/retina` in a retinal-detachment practical/viva subsection; `/osce` in its long-case list; `/exams/dnb` beside practical preparation; `/products/retina-deciphered` alongside retinal-case revision material. Do not create an unpublished `/blog/` link until the blog sitemap verifies it.

## Sources

[^statpearls]: Blair K, Czyz C. [Retinal Detachment](https://www.ncbi.nlm.nih.gov/books/NBK551502/). *StatPearls*; last updated 12 February 2024. Used for the definition, three mechanisms, examination concepts and case-specific management determinants. URL checked 18 August 2026.

[^pvr]: Machemer R, Aaberg TM, Freeman HM, Irvine AR, Lean JS, Michels RM. [An updated classification of retinal detachment with proliferative vitreoretinopathy](https://pubmed.ncbi.nlm.nih.gov/1867299/). *American Journal of Ophthalmology*. 1991;112(2):159–165. Used for the PVR classification context. PubMed record URL checked 18 August 2026.

## QA

**Factuality 5/5:** clinical classification and management determinants are source-bound; exam advice is labelled as a framework. **Search Intent 5/5:** directly answers the long-case presentation and viva query. **Reader Value 5/5:** provides a spoken opening, examination sequence, differential table, PVR language and viva structures. **Information Gain 5/5:** replaces a generic “what is RD” explainer with a retinal-map and uncertainty-safe presentation workflow. **Competitive Superiority 4/5:** practical case-map method; no unsupported claims. **SEO 5/5:** direct answer, precise H2s and keyword-aligned metadata. **GEO 5/5:** extractable answer block, tables and source attribution. **Naturalness 5/5:** doctor-to-doctor, no filler. **E-E-A-T 4/5:** independent clinical references; final pre-publication guideline check remains necessary. **Experience Integrity 5/5:** no fabricated personal experience or outcome claim. **Internal Linking 5/5:** four verified `.org` destinations, each contextual. **Conversion 4/5:** matching resource is optional and secondary. **Freshness 4/5:** source URLs checked 18 August 2026; clinical/exam details require recheck before publication.
