...
primary_keyword: "eyedocs eyeq review"
target_page: "/compare/best-ophthalmology-question-banks"
...
1. Which error type recurred most often?
2. Did the resource’s explanations identify a repair source quickly enough?
3. Is next week’s assessment task still written retrieval, or has it moved to refraction/oral performance?
...
1. Open the official RCOphth page and write the next component and its stated assessment purpose.
2. Open the supplier page for the exact bank, not the home page. Confirm its name and stated scope match that component.
3. Check current access, price, cancellation/refund terms and supported workflow on supplier purchase material. Do not use this article as the source for mutable facts.
4. Complete a sample block if one is currently offered. Time it.
5. Mark every error as knowledge, discrimination, stem-reading or pacing. Read at least five explanations in full.
6. Identify the second resource needed for errors that the bank cannot repair: textbook, notes, mentor session, clinical practice or spoken cases.
7. Decide only after you can state what you will do with the resource on Monday morning.
...
- **Authority boundary:** RCOphth source used for qualification-route and assessment-purpose statements. Supplier pages used only for attributed, current product framing.
- **Withheld mutable facts:** supplier pricing, access period, question counts, device/app availability, subscription terms, refund terms, feature availability, update policy and product inventory beyond named pages.
- **Withheld promotional claims:** testimonials, pass/score predictions, rankings and superiority language.
- **Final editor check:** revisit RCOphth rules and each supplier’s exact purchase page before publication; validate the final theStacc canonical and all links after upload.

### Internal-link manifest

| Source section | Destination | Anchor text | Link class | Purpose | Priority |
|---|---|---|---|---|---|
| Decision in one minute | `/exams/frcophth` | OphthaMCQ FRCOphth preparation guide | BODY_CONTEXTUAL | Component-planning orientation | P0 |
| What OphthaMCQ documents | `/products/frcophth-step-1-mcqs` | FRCOphth Step 1 MCQs | BODY_CONTEXTUAL | Direct product-scope verification | P1 |
| What OphthaMCQ documents | `/free-mcqs/frcophth` | FRCOphth free MCQs | BODY_CONTEXTUAL | Low-commitment sample route | P1 |
| Fair comparison | `/compare/best-ophthalmology-question-banks` | broader ophthalmology question-bank comparison | BODY_CONTEXTUAL | Lateral provider discovery without duplication | P1 |

**Inbound opportunities after publication:** `/resources` (resource-selection guide), `/exams/frcophth` (question-bank selection context), `/compare/best-ophthalmology-question-banks` (FRCOphth-specific decision aid), and `/faq` (how to choose a bank). Add links only after theStacc has published this article and its URL is confirmed in the sitemap.

### Gate result

| Gate | Score | Rationale |
|---|---:|---|
| Factuality | 5/5 | Official exam source separated from supplier claims; mutable facts withheld. |
| Search intent | 5/5 | Directly answers a comparison query with a purchase decision process. |
| Reader value | 5/5 | Provides a component matrix, trial protocol and error-log workflow. |
| Information gain | 5/5 | Avoids generic winner tables and isolates written, refraction and oral tasks. |
| SEO | 5/5 | Exact entity/keyword coverage, direct answer and comparison structure. |
| GEO | 5/5 | Source-attributed answer blocks and extractable tables. |
| Naturalness | 5/5 | Specific, peer-to-peer revision language; no hype or outcome promise. |
| Experience integrity | 5/5 | No invented testing, personal experience or customer evidence. |
| Internal linking | 5/5 | Four validated `.org` routes, contextual anchors and inbound plan. |
| Conversion | 4/5 | Suggests a relevant sample/product only when component fit is demonstrated. |
| Freshness | 4/5 | Sources checked 18 August 2026; final purchase and exam-rule check required. |
