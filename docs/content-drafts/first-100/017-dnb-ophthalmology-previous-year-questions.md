...
1. **Freeze the recall.** Copy it exactly as received and label the source. Do not silently repair a missing detail.
2. **State the concept in one sentence.** For example: “This is testing the relationship between a visual-field defect and lesion location,” not “the answer is B.”
3. **Verify from a standard source.** Check the mechanism, definition, classification or calculation. If the recollection conflicts with the reference, the reference wins.
4. **Create a neighbouring comparison.** Ask what single feature would reverse the answer. That is where SBA/MCQ distractors usually live.
5. **Re-test in a new form.** Use a blank diagram, altered values, a different image or an oral explanation. If you only recognise the old wording, you have memorised a recall rather than learned a principle.
...
- [NBEMS: DNB/DrNB Exit Examinations](https://natboard.edu.in/viewnbeexam?exam=dnb) — current official Final examination process; checked 18 August 2026.
- [NBEMS curriculum library](https://nbe.edu.in/curriculum.html) — official curriculum documents, including Ophthalmology; checked 18 August 2026.
- [NBEMS practical examination portal](https://exam.natboard.edu.in/candidates/viewpracticalexam.php) — session-specific practical schedule and candidate information; checked 18 August 2026.

## Final QA and link manifest

**Education boundary:** for postgraduate exam preparation only; this is not patient-care advice, an NBEMS endorsement or a prediction of an examination result. **Claim check:** official examination-process facts are linked above; no numerical repeat rate, pass rate, marks or fixed weightage is asserted. **Outbound verified internal links:** `/exams/dnb` (DNB Ophthalmology preparation hub; BODY_CONTEXTUAL; P0), `/products/dnb-do-past-5-year-papers` (Past 5-Year Question Papers DNB/DO; BODY_CONTEXTUAL; P1), `/osce` (OSCE, Practical & Viva Voce Ready bundle; BODY_CONTEXTUAL; P0). **Inbound opportunities:** `/exams/dnb` past-papers section, `/resources` revision-method list, `/products/dnb-do-past-5-year-papers` usage guidance. No blog destination is used because publication is unverified.
